...
export class ReactiveFormComponent implements OnInit, OnDestroy {
  ...
  private createForm(): void {
    this.form = new FormGroup({
      ...
      message: new FormControl({value: 'message', disabled: true}, [this.messageValidator]),
      name: new FormControl('my name', [Validators.minLength(4)]),
      email: new FormControl(null, [Validators.required, Validators.email]),
      favoriteProgrammingLanguage: new FormControl({value: 'c#', disabled: false}),
  ...
  private validateControl(controlName: string): boolean {
    ...
    return control != null && (control.untouched || (control.valid && control.touched));
  }

  private subscribeFormControls(): void {
    this.form.get('customMessage')?.valueChanges
      .pipe(
      ...
      this.form.get('favoriteProgrammingLanguage')?.valueChanges
      .pipe(
        takeUntil(this.onDestroy$)
      )
